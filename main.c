                          ...
                          char cmd[3][TOKEN_MAX_LEN], int token_count)
{
    ...
    {
        char *aux = malloc(strlen(cmd[1]) + 1);
        memcpy(aux, cmd[1], strlen(cmd[1]) + 1);
        mkdir(currentFolder, aux);
        free(aux);
    }
    ...
    {
        char *aux1 = malloc(strlen(cmd[1]) + 1);
        memcpy(aux1, cmd[1], strlen(cmd[1]) + 1);
        char *aux2 = malloc(strlen(cmd[2]) + 1);
        memcpy(aux2, cmd[2], strlen(cmd[2]) + 1);
        touch(currentFolder, aux1, aux2);
        free(aux1);
        free(aux2);
    }
...
int main()
{
    ...
    char *token;
    char root_name[] = "root";
    FileTree fileTree = createFileTree(root_name);
